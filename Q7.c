...
int main(void)
{
	...
	char * commandeSansRedirection;
	char * emplacementRedirectionSortie;
	char * emplacementRedirectionEntree;
	
	int argc = 10; //On estime qu'on n'aura pas plus d'arguments
	char * argv[argc];
	argv[9] = NULL;
	...
	while(1){
		...
		//On teste si l'on a un chevron dans la commande
		commandeSansRedirection = strtok(commande, ">"); //Test du premier chevron
		emplacementRedirectionSortie = strtok(NULL,">");
		emplacementRedirectionEntree = strtok(commande,"<"); //Test du second chevron
		commandeSansRedirection = strtok(NULL, "<");


		//S'il y a un chevron, alors on execute la redirection
		if(emplacementRedirectionSortie != NULL){ 
			int nouveaufd = open(emplacementRedirectionSortie, O_WRONLY); //Ouverture du fichier dans lequel on va écrire
			dup2(nouveaufd,STDOUT_FILENO); //Duplique le descripteur du fichier
			close(nouveaufd);
		}
		else if(emplacementRedirectionEntree != NULL){ 
			int nouveaufd = open(emplacementRedirectionEntree, O_RDONLY); //Ouverture du fichier dans lequel on va écrire
			dup2(nouveaufd,STDIN_FILENO); //Duplique le descripteur du fichier
			close(nouveaufd);
		}
		
		
		
		//On stocke les différents mots dans le tableau argv
